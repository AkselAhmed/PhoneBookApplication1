...
CDataBaseConnection* CDataBaseConnection::m_pInstance = NULL;
...
BOOL CPhoneBookApp::InitInstance()
{
	...
	// Свързваме се с базата данни при отваряне на приложението
	...
	pDocTemplate = new CMultiDocTemplate(IDR_CitiesView,
	...
	pDocTemplate = new CMultiDocTemplate(IDR_PersonsView,
	...
	pDocTemplate = new CMultiDocTemplate(IDR_PhoneTypesView,
...
int CPhoneBookApp::ExitInstance()
{
	...
	// При излизане от приложението затваряме връзката с базата данни и изтриваме заделената памет
	CDataBaseConnection* pDataBaseConnection = CDataBaseConnection::GetInstance();
	pDataBaseConnection->ReleaseInstance();
